...
command.add(function()
  local node = core.active_window().root_view:get_active_node()
  local sx, sy = node:get_locked_size()
...
command.add(nil, {
  ["root:scroll"] = function(delta)
    local view = core.active_window().root_view.overlapping_view or core.active_view
...
command.add(function(node)
    ...
    node = node or core.active_window().root_view:get_active_node()
...
command.add(function()
    local node = core.active_window().root_view.root_node:get_child_overlapping_point(core.active_window().root_view.mouse.x, core.active_window().root_view.mouse.y)
    if not node then return false end
...
command.add(function(x, y)
  local node = x and y and core.active_window().root_view.root_node:get_child_overlapping_point(x, y)
