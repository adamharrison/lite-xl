...
local function doc(rv)
  local docview = dv(rv)
  return docview and docview.doc
end
...
local function find(dv, label, search_fn)
  last_view, last_sel = dv, { dv:get_selection() }
...
local function select_next(dv, reverse)
  ...
  local text = dv.doc:get_text(l1, c1, l2, c2)
  if reverse then
...
local function find_replace(dv, in_selection)
  ...
  if not in_selection then
    selected_text = dv.doc:get_text(l1, c1, l2, c2)
...
command.add("core.docview!", {
  ...
  ["find-replace:replace-symbol"] = function(dv)
    ...
    if dv:has_selection() then
      local text = dv.doc:get_text(dv:get_selection())
