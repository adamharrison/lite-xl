...
function TitleView:configure_hit_test(borderless)
  if borderless then
    ...
    system.set_window_hit_test(self.window.renwindow, title_height, controls_width, icon_spacing)
    -- core.hit_test_title_height = title_height
  else
    system.set_window_hit_test(self.window.renwindow)
  end
end
...
function TitleView:draw_window_title()
  ...
  local title = self.window:compose_window_title(self.window.title)
