...
main() {
  ...
  if [[ $addons != "" ]]; then
    ...
    $build_dir/lpm init https://github.com/adamharrison/lite-xl-plugin-manager.git:3.0 --userdir ${build_dir}/src/data
    "$build_dir/lpm$(get_executable_extension)" install --datadir ${build_dir}/src/data --userdir ${build_dir}/src/data --arch $(get_platform_tuple) $plugins --assume-yes --mod-version=4; "$build_dir/lpm$(get_executable_extension)" purge --datadir ${build_dir}/src/data --userdir ${build_dir}/src/data && chmod -R a+r ${build_dir}
  fi
